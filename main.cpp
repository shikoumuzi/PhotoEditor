...
int main(int argc, char *argv[])
{
    ...
    ////image.Rgb2Gray(error_no);
    //image.thresold(error_no, 125, Coordinates(&image, image.rows() / 3, image.cols()/3), Coordinates(&image, image.rows()/2, image.cols()/2));
    ...
    //image.offsetAll(error_no, 120, Coordinates(image, image.rows() / 4, image.cols() / 3), Coordinates(image, image.rows() / 3, image.cols() / 2));
